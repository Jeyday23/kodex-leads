...
export async function generateStaticParams() {
  ...
  return pages
    .filter((page) => page.pageType === "enforcement" && page.framework)
    .map((page) => ({ framework: page.framework as string, slug: page.slug }));
}

export async function generateMetadata({ params }: RouteProps): Promise<Metadata> {
  const { framework, slug } = await params;
  const page = await getSeoPageByRoute("enforcement", slug, framework);
  if (!page) return {};
...
export default async function EnforcementPage({ params }: RouteProps) {
  const { framework, slug } = await params;
  const page = await getSeoPageByRoute("enforcement", slug, framework);
  if (!page) notFound();
